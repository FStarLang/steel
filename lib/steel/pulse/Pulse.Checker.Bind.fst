...
let check_stapp_no_ctxt (g:env) (t:st_term)
...
let check_bindv2
  ...
  let Tm_Bind { binder=b; head=e1; body=e2 } = t.term in
    ...
    // if not (stateful_comp c2) then fail g None "Bind: c2 is not st";
    ...
    //                  (PS.nt_subst_comp c1 ss1) = PS.st_typing_nt_substs_derived g1 uvs1 #e1 #c1 d1 ss1 in
    let d1
      : st_typing g1 (PS.nt_subst_st_term e1 ss1)
    ...
    //   = coerce_eq d2 () in
    // let post_hint1 : post_hint_opt g1 = extend_post_hint_opt_g g post_hint g1 in
    // assume (comp_post_matches_hint c2 post_hint1);
    ...
    //   coerce_eq d2 () in
    // let post_hint : post_hint:post_hint_opt g { comp_post_matches_hint c2 post_hint } = magic () in
