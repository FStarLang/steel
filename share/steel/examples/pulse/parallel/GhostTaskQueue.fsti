...
let is_mono_suffix (#a: Type): P.preorder (mono_list a) =
...
let rec task_in_queue (#a: Type) (x: task_plus a) (pos:nat) (l: mono_list a): prop =
...
let rec task_in_queue_same_mono #a (t: task_plus a) (pos:nat) (x y: mono_list a):
...
let rec task_in_queue_preorder #a 
    (t: task_plus a) (pos: nat) (x y: mono_list a):
...
let enqueue_todo #a (l: mono_list a) (t: task_plus a)
(*
...
let enqueue_preserves_order #a l (t: task_plus a):
...
let get_actual_queue #a (l: mono_list a): list a =
    L.map (fun (t: extended_task a) -> t._1._1) (L.filter is_Todo l)
...
: (t:task_plus a & r:mono_list a{ get_actual_queue l == t._1::get_actual_queue r
...
val mono_share2 (#a:Type) (r:ghost_mono_ref a) (#v:erased (mono_list a))
  : stt_ghost unit
      (M.pts_to r full_perm v)
      (fun _ -> M.pts_to r one_half v ** M.pts_to r one_half v)

val mono_gather2 (#a:Type) (r:ghost_mono_ref a) (#x0 #x1: erased (mono_list a))
  : stt_ghost unit
...
= exists_ (fun l -> M.pts_to r full_perm l **
  tasks_res_own l **
...
requires (exists l. M.pts_to r one_half l ** tasks_res_own l f **
      pure (count_ongoing l = 0 /\ get_actual_queue l == []))
  ensures deadline r
{
  with l. assert (M.pts_to r one_half l **
      pure (count_ongoing l = 0 /\ get_actual_queue l == []));
  unfold M.pts_to r one_half l;
...
= exists_ (fun l -> M.pts_to r one_half l **
...
ensures exists l. (M.pts_to r full_perm l
...
)
{
  ...
  rewrite `@(if c = 0 && L.length q = 0 then deadline r
    ...
    as (M.pts_to r one_half l ** tasks_res_own l one_quart);
  //share_queue_general r l ;
  rewrite (pts_to_ghost_queue_half (reveal (hide r)) (reveal l))
    as (M.pts_to r one_half (reveal l));
