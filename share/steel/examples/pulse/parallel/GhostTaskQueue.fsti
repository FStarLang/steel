...
```pulse
unobservable fn add_todo_task_to_queue (r: ghost_mono_ref) (i: inv (inv_ghost_queue r)) (t: task)
...
val assume_prop (p: prop): stt_ghost unit emp (fun () -> pure p)
...
```pulse
unobservable fn pop_task_ghost
...
```pulse
atomic fn conclude_task (t: task) (pos: nat)
