...
let test2 #a #b (l: mono_list a):
  Lemma
  (requires ~(L.map fst (L.filter (fun t -> Todo? (snd t)) l) == []))
  (ensures ~(l == []))
= ()
...
let isTodo #a (t: task_with_status a): bool
  = Todo? (snd t)

let get_actual_queue #a (l: mono_list a): list a =
    L.map fst (L.filter isTodo l)

//let map_non_empty #a #b (l: list a) (f: a -> b):
//  Lemma (requires ~(L.map f l == []))
// (ensures ~(l == []))
//= ()

// map_non_empty (L.filter (fun t -> Todo? (snd t)) l) fst
...
let rec pop_todo_task #a (l: mono_list a{~(L.isEmpty (get_actual_queue l))})
...
ensures pts_to_ghost_queue r l1 ** pure (l1 == l2)
{
  ...
  M.gather r one_half one_half l1 l2;
...
let alloc_ghost_queue (#a:Type)
  : stt_ghost (ghost_mono_ref a) emp (fun r -> pts_to_ghost_queue r [])
= M.alloc #_ #is_mono_suffix []

let write_ghost_queue (#a:Type) (#v: mono_list a)
          (r:ghost_mono_ref a) (x: mono_list a)
  : stt_ghost unit (pts_to_ghost_queue r v ** pure (squash (is_mono_suffix v x)))
...
let pool = (GR.ref bool & ghost_mono_ref task_elem)

let certificate (r:pool) (t: task_elem) (pos: nat)
= M.witnessed (snd r) (task_in_queue t pos)
...
let get_certificate (t: task_elem) (pos: nat)
  ...
  stt_ghost (certificate r t pos)
                  (pts_to_ghost_queue (snd r) l)
                  (fun _ -> pts_to_ghost_queue (snd r) l)
= witness (snd r) l _

let pts_to_Some (t: task_elem & task_status) (f: perm): vprop
= (exists* v. pts_to (t._1._2) #f v ** pure (Some? v))
...
// assuming because of universe polymorphism
assume val
stt_return_ghost #a (x:a): stt_ghost a emp (fun y -> pure (x == y))

(*
// Assuming recursion
assume val share_tasks_res_own_generic_cb (p: mono_list_task_plus) (f1: perm) (f2: perm):
stt_ghost unit (tasks_res_own p (sum_perm f1 f2))
...
decreases l
{
  if (L.length l = 0)
  {
    ...
    share_tasks_res_own_generic ql f1 f2;
...
: stt_ghost unit (tasks_res_own l f1 ** tasks_res_own l f2)
(fun () -> tasks_res_own l (sum_perm f1 f2))
*)

```pulse
ghost fn rec gather_tasks_res_own (l: mono_list_task_plus) (f1 f2: perm)
...
let deadline (r: pool): vprop
= exists* f1 f2 l. 
    M.pts_to (snd r) f1 l ** tasks_res_own l f2
    ** pure (count_ongoing l = 0 /\ get_actual_queue l == [])

```pulse
ghost fn
duplicate_deadline' (r: pool)
requires deadline r
ensures deadline r ** deadline r
{
  unfold deadline r;
  with f1 f2 l. assert (M.pts_to (snd r) f1 l ** tasks_res_own l f2
    ** pure (count_ongoing l = 0 /\ get_actual_queue l == []));
  M.share (snd r) f1 l;
...
```pulse
ghost fn
obtain_deadline (r: pool) (f: perm)
requires (exists* l. pts_to_ghost_queue_half (snd r) l ** tasks_res_own l f **
      pure (count_ongoing l = 0 /\ get_actual_queue l == []))
  ensures deadline r
{
  with l. assert (pts_to_ghost_queue_half (snd r) l **
      pure (count_ongoing l = 0 /\ get_actual_queue l == []));
  unfold pts_to_ghost_queue_half (snd r) l;
  assert (M.pts_to (snd r) one_half l ** pure (count_ongoing l = 0 /\ get_actual_queue l == []));
  fold deadline r;
  ()
}
```
