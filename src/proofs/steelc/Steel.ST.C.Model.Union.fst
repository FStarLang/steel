...
: ST (ref base (p k))
    ...
    (requires ~ (Ghost.reveal xs k == one (p k)))
    (ensures fun r' -> r' == ref_focus r (union_field p k))
= union_peel p k xs;
  rewrite (r `pts_to` xs) (r `pts_to` _);
  let s = focus r (union_field p k) (field_to_union_f p k (Ghost.reveal xs k)) (Ghost.reveal xs k) in
  rewrite (s `pts_to` _) (s `pts_to` _);
  return s
...
: STGhost unit opened
    ...
    (requires r' == ref_focus r (union_field p k))
    (ensures fun _ -> True)
