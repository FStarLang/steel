...
type comp_st = comp
type pattern =
  | Pat_Cons of fv * (FStar_Reflection_Typing.pp_name_t * Prims.bool)
  Prims.list 
...
let (__proj__Pat_Cons__item___1 :
  pattern -> (FStar_Reflection_Typing.pp_name_t * Prims.bool) Prims.list) =
...
let (eq_pat_arg :
  (FStar_Reflection_Typing.pp_name_t * Prims.bool) ->
    (FStar_Reflection_Typing.pp_name_t * Prims.bool) -> Prims.bool)
  =
  fun p1 ->
    fun p2 ->
      let uu___ = p1 in
      match uu___ with
      | (n1, b1) ->
          let uu___1 = p2 in (match uu___1 with | (n2, b2) -> b1 = b2)
let (eq_pattern : pattern -> pattern -> Prims.bool) =
  fun p1 ->
    fun p2 ->
      match (p1, p2) with
      | (Pat_Cons (f1, vs1), Pat_Cons (f2, vs2)) ->
          (f1.fv_name = f2.fv_name) && (eq_list_dec p1 p2 eq_pat_arg vs1 vs2)
