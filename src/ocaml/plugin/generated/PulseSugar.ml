...
let (uu___is_NOW : mut_or_ref -> Prims.bool) =
  fun projectee -> match projectee with | NOW -> true | uu___ -> false
...
let (__proj__Mkstmt'__LetBinding__payload__item__isnow :
  stmt'__LetBinding__payload -> Prims.bool) =
  fun projectee ->
    match projectee with | { isnow; qualifier; id; typ; init;_} -> isnow
...
let (mk_let_binding :
  ...
  fun isnow ->
    fun qualifier ->
      fun id ->
        fun typ -> fun init -> LetBinding { isnow; qualifier; id; typ; init }
